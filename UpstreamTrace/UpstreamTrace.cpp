// UpstreamTrace.cpp : 定義主控台應用程式的進入點。
//
...
int main(int argc, char **argv){
	...
	while(true){
		...
		if(sConnect = accept(sListen, (SOCKADDR*)&clinetAddr, &addrlen)){
			...
			traceUpstream(pldata, cx, cy);

			const char *sendback = "Traceback Done";
			int length = sizeof("Traceback Done");
            send(sConnect, sendback, length, 0);
        }
	}
